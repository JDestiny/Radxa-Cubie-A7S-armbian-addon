...
#   * 只做一件事：把 img-bxm-dkms-src-gpuacct/ 这份**打过补丁**的驱动（V6 = 无锁热路径 + 条目回收 + fdinfo 去重）编译并**持久化安装**为 pvrsrvkm
#   * 与 install-gpu-driver-stock.sh（stock = Radxa r6 原版，htop 看不到 GPU）**完全独立**：两个脚本各自自包含，
#     装哪个由你选；本脚本会把另一个变体从 DKMS 树里摘掉（避免内核升级时双份竞争）
#   * 持久化：DKMS（0.1.0-3+gpuacct）+ 开机 /etc/modules-load.d/pvr.conf 按名加载 + AUTOINSTALL=yes（换内核自动重建）
#
# 用法:
#   sudo ./install-gpu-driver-gpuacct.sh              # 安装/重装（总是重新编译，约 1~2 分钟）
#   sudo ./install-gpu-driver-gpuacct.sh --status     # 只读：看「已加载 / DKMS / 磁盘 ko(=重启后加载谁)」
#   sudo ./install-gpu-driver-gpuacct.sh --reload     # 只重载模块，不重装
#   sudo ./install-gpu-driver-gpuacct.sh --help
#
...
KREL="$(uname -r)"
SRC="$BASE/img-bxm-dkms-src-gpuacct"
...
if [ "${1:-}" = "--status" ]; then
    echo "=============================================="
    echo " V6 GPU 驱动安装器（原版 + GPU 利用率记账补丁，htop 直接可用）"
    echo "=============================================="
    echo "  本脚本变体 : gpuacct  (DKMS 0.1.0-3+gpuacct)"
    ...
    echo "  内核       : $KREL"
    echo "  源         : $SRC"
    echo "  说明       : 装完 htop 即可看到 GPU 表头与进程 GPU%/GPU TIME（每 ~6s 跳一次; 数值偏大属 htop 口径，见 GPU 文档）"
    exit 0
fi
...
echo "=============================================="
echo " 安装 GPU 驱动：V6 记账补丁版（DKMS 0.1.0-3+gpuacct）"
echo "=============================================="
...
if dkms status "$DKMS_NAME/$OTHER_VER" 2>/dev/null | grep -q .; then
    info "移除另一变体 $DKMS_NAME/$OTHER_VER (原版 stock)"
...
if dkms install "$DKMS_NAME/$VER" -k "$KREL" >/tmp/dkms-gpu-install-gpuacct.log 2>&1; then
    ok "DKMS 安装成功 ($DKMS_NAME/$VER)"
else
    bad "DKMS 安装失败 (见 /tmp/dkms-gpu-install-gpuacct.log):"
    tail -5 "/tmp/dkms-gpu-install-gpuacct.log" | sed 's/^/         /'
...
if [ "$now" = "gpuacct" ] && [ "$disk" = "gpuacct" ] && [ "$dk" = "0.1.0-3+gpuacct" ]; then
    ok "持久化: 运行中 = 磁盘 ko = DKMS(0.1.0-3+gpuacct) = gpuacct"
    ok "重启后仍为 gpuacct (由 /etc/modules-load.d/pvr.conf 在开机时按名加载)"
else
    bad "状态不一致: 运行中=$now 磁盘=$disk DKMS=$dk (期望都是 gpuacct/0.1.0-3+gpuacct)"
fi
if [ "$now" = "gpuacct" ]; then
    info "自检: 跑个 GPU 程序后 grep drm-engine-pvr /proc/<pid>/fdinfo/*"
fi
echo "──────────────────────────────────────────────"
if [ "$FAIL" = "0" ]; then
    echo " 完成: 当前 GPU 驱动 = V6 记账补丁版（DKMS 0.1.0-3+gpuacct） ✅"
else
    echo " 存在失败项 ❌"
fi
